...
DATASETS = {
    ...
    "ftse": {"tickers_file": BASE_DIR / "tickers_ftse.csv", "output_file": BASE_DIR / "results_ftse.csv", "max_tickers": 500, "benchmark": "^FTSE"},
    "us": {"tickers_file": BASE_DIR / "tickers_us.csv", "output_file": BASE_DIR / "results_us.csv", "max_tickers": 500, "benchmark": "SPY"},
    "europe": {"tickers_file": BASE_DIR / "tickers_europe.csv", "output_file": BASE_DIR / "results_europe.csv", "max_tickers": 500, "benchmark": "STOXX50E"},
}
...
def process_ticker(ticker):
    try:
        ...
        top_features = ", ".join(pd.Series(model.feature_importances_, index=feature_cols).sort_values(ascending=False).head(3).index.tolist())
        score = score_row(pd.Series({"Predicted Prob Up (%)":prob_up,"Accuracy (%)":accuracy_pct,"Precision (%)":precision_pct,"F1 Score (%)":f1_pct,"RSI_14":latest_rsi}))
        ...
        return {"result":{"Ticker":ticker,"Latest Price ($)":latest_price,"Daily Return":latest_return_1d,"Volatility":latest_volatility_30,"Predicted Prob Up (%)":prob_up,"Accuracy (%)":accuracy_pct,"Precision (%)":precision_pct,"Recall (%)":recall_pct,"F1 Score (%)":f1_pct,"RSI_14":latest_rsi,"Top Features":top_features,"Score":score,"Rating":rating_from_score(score)},"perf":{"Ticker":ticker,"pick_prob_up":prob_up,"pick_score":score,"start_close":latest_price,"end_close_5d":end_close_5d,"forward_return_5d_pct":forward_return_5d_pct,"hit":hit}}
...
def refresh_dataset(name, cfg):
    ...
    if not results:
        raise RuntimeError(f"No results generated for {name}")
    pd.DataFrame(results).sort_values(["Score","Predicted Prob Up (%)"], ascending=False).to_csv(cfg["output_file"], index=False)
    return perf_rows
...
def build_weekly_summary():
    ...
    summary = perf.groupby(["market","week_start"], dropna=True).agg(picks=("Ticker","count"),hit_rate=("hit","mean"),avg_return_5d_pct=("forward_return_5d_pct","mean"),avg_pick_prob_up=("pick_prob_up","mean"),avg_pick_score=("pick_score","mean")).reset_index()
...
def build_benchmark_summary():
    ...
    for market, cfg in DATASETS.items():
        ...
        bench_weekly = bench.groupby("week_start", dropna=True).agg(benchmark_return_5d_pct=("forward_return_5d_pct","mean")).reset_index()
