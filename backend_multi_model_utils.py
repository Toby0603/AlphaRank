...
def process_ticker(ticker, model_name, feature_set_name):
    try:
        ...
        print(f"Failed {ticker} [{model_name}/{feature_set_name}]: {e}")
        return {"failed": True, "ticker": ticker, "reason": str(e)}


def append_tracker_rows(rows):
    if not rows:
        return
    ...
    key_cols = ["pick_date", "Ticker", "start_close", "model", "feature_set"]
    ...
    if all(col in combined.columns for col in key_cols):
        ...
        combined["feature_set"] = combined["feature_set"].astype(str).str.strip().str.lower()
        ...
        combined = combined.sort_values(
            by=key_cols + ["_has_end_close", "_has_forward_ret", "_has_hit", "_row_order"],
            ascending=True,
        )
...
def append_failed_rows(rows):
    ...
    dedupe_cols = [c for c in ["Ticker", "model", "feature_set", "Reason"] if c in combined.columns]
    if dedupe_cols:
...
def save_comparison_summary(all_results):
    ...
    for variant_name, df in all_results.items():
        ...
        row = {
            ...
            "avg_precision": df["Precision (%)"].mean() if "Precision (%)" in df.columns else None,
...
def run_variant(model_name, feature_set_name, output_file):
    ...
    if results:
        ...
        print(f"No valid results for {variant_name}")

    append_tracker_rows(tracker_rows)
    append_failed_rows(failed_rows)

    print(f"Appended {len(tracker_rows)} tracker rows for {variant_name}")
    print(f"Appended {len(failed_rows)} failed rows for {variant_name}")
